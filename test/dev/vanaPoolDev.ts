...
describe("VanaPool", () => {
  ...
  function formatTimeInterval(seconds: number): string {
    ...
    // Only use the parts that are non-zero
    const parts: string[] = [];
    ...
    // Format the output with appropriate conjunctions
    if (parts.length === 1) {
      return parts[0];
  ...
  describe("calculateYield", () => {
    ...
    const calculateYieldTestCases = [
      {
        amount: parseEther(1),
        apy: parseEther(6),
        timeElapsed: year,
        expectedReward: parseEther("0.061836546545359625"),
      },
      {
        amount: parseEther(2),
        apy: parseEther(6),
        timeElapsed: year,
        expectedReward: parseEther("0.123673093090719244"),
      },
      {
        amount: parseEther(1),
        apy: parseEther(6),
        timeElapsed: 2 * year,
        expectedReward: parseEther("0.127496851579375671"),
      },
      {
        amount: parseEther(100),
        apy: parseEther(6),
        timeElapsed: year,
        expectedReward: parseEther("6.1836546545359622"),
      },
      {
        amount: parseEther(1000),
        apy: parseEther(6),
        timeElapsed: year,
        expectedReward: parseEther("61.836546545359622"),
      },
      {
        amount: parseEther(0.01),
        apy: parseEther(6),
        timeElapsed: year,
        expectedReward: parseEther("0.000618365465453596"),
      },
      {
        amount: parseEther(1),
        apy: parseEther(6),
        timeElapsed: day,
        expectedReward: parseEther("0.000164397073361862"),
      },
      {
        amount: parseEther(1),
        apy: parseEther(6),
        timeElapsed: 10 * day,
        expectedReward: parseEther("0.001645187454837179"),
      },
      {
        amount: parseEther(1),
        apy: parseEther(6),
      ...
      },
      {
        ...
        apy: parseEther(6),
        timeElapsed: 1,
        expectedReward: parseEther("0.000000001902587520"),
      },
    ];

    calculateYieldTestCases.forEach(
      ({ amount, apy, timeElapsed, expectedReward }) => {
        it(`should calculateYield (${formatEther(amount)} VANA, ${formatEther(apy)}%, ${formatTimeInterval(timeElapsed)})`, async function () {
          const yieldAmount = await vanaPoolEntity.calculateYield(
            amount,
            apy,
            timeElapsed,
          );
          yieldAmount.should.closeTo(expectedReward, 10);
        });
      },
    );

    const multipleCalculateYieldTestCases = [
      {
        amount: parseEther(1),
        apy: parseEther(6),
        timeElapsed: 1,
    ...
    multipleCalculateYieldTestCases.forEach(({ amount, apy, timeElapsed }) => {
      it(`should call calculateYield multiple times (${formatEther(amount)} VANA, ${formatEther(apy)}%, ${formatTimeInterval(timeElapsed)})`, async function () {
        const yieldAmount1 = await vanaPoolEntity.calculateYield(
          amount,
          apy,
          timeElapsed,
        );

        const yieldAmount2 = await vanaPoolEntity.calculateYield(
          amount + yieldAmount1,
          apy,
          timeElapsed,
        );

        const yieldAmount3 = await vanaPoolEntity.calculateYield(
          amount,
          apy,
          2 * timeElapsed,
        );

        yieldAmount3.should.closeTo(yieldAmount2 + yieldAmount1, 10);

        let yield5 = 0n;

        for (let i = 1; i <= 10; i++) {
          yield5 += await vanaPoolEntity.calculateYield(
            amount + yield5,
            apy,
            timeElapsed,
          );
        ...
        const yield6 = await vanaPoolEntity.calculateYield(
          amount,
          apy,
          10 * timeElapsed,
        );

        yield5.should.closeTo(yield6, 10000);
        yield5.should.lt(yield6);
      });
    });
  });
  ...
  describe("Staking Operations with lockedPoolAmount = 0", () => {
    ...
    it("should unstake from an entity", async function () {
      ...
      const unstakeSharesAmount = userShares / 4n;
      ...
      const user2BalanceAfter = await ethers.provider.getBalance(user2);
      user2BalanceAfter.should.eq(
        user2BalanceBefore - receipt.fee + stakeAmount / 4n,
      );
      ...
      remainingShares.should.eq((userShares * 3n) / 4n);
    });
  ...
  describe("Staking Operations with lockedPoolAmount != 0", () => {
    ...
    const initialLockedPoolAmount = parseEther(1000);
    ...
    beforeEach(async () => {
      ...
      await vanaPoolEntity
        .connect(owner)
        .addRewards(1, { value: initialLockedPoolAmount });
    });

    const testCases = [
      {
        description: "after 1 year",
        timeElapsed: year,
        expectedReward: parseEther("0.061836546545359625"),
      },
      {
        description: "after 2 years",
        timeElapsed: 2 * year,
        expectedReward: parseEther("0.127496851579375671"),
      },
    ];

    testCases.forEach(({ description, timeElapsed, expectedReward }) => {
      it(`should process ${description}`, async function () {
        await advanceToTimestamp(entityCreationTimestamp + timeElapsed);
        await vanaPoolEntity.processRewards(1);

        const entity = await vanaPoolEntity.entities(1);

        entity.activeRewardPool.should.closeTo(
          minRegistrationStake + expectedReward,
          10,
        );
        entity.lockedRewardPool.should.closeTo(
          initialLockedPoolAmount - expectedReward,
          10,
        );
      });
    });

...
///         and state, forked at the latest block; nothing is broadcast). Run with:
...
contract MokshaE2ETest is Test {
    ...
    VanaPoolStakingImplementation constant staking = VanaPoolStakingImplementation(payable(0x641C18E2F286c86f96CE95C8ec1EB9fC0415Ca0e));
    VanaPoolEntityImplementation constant entity = VanaPoolEntityImplementation(payable(0x44f20490A82e1f1F1cC25Dd3BA8647034eDdce30));
    VanaPoolTreasuryImplementation constant treasury = VanaPoolTreasuryImplementation(payable(0x143BE72CF2541604A7691933CAccd6D9cC17c003));
    RewardSplitterImplementation constant splitter = RewardSplitterImplementation(payable(0x7A7B89b6925A8156b9A51E520327c0701023b344));
    ...
    function setUp() public {
        ...
        vm.createSelectFork(vm.envOr("MOKSHA_RPC_URL", string("https://rpc.moksha.vana.org")));
        ...
        owners[0] = makeAddr("owner-alpha"); owners[1] = makeAddr("owner-bravo"); owners[2] = makeAddr("owner-charlie");
    ...
    function _owed() internal view returns (uint256 sum) {
        ...
        for (uint256 i = 1; i <= n; i++) {
            ...
            sum += e.activeRewardPool + e.lockedRewardPool + entity.entityAccruedCommission(i) + entity.entityStakerLockedRewardPool(i);
        }
    }
    ...
    function _ids() internal view returns (uint256[] memory ids) {
        ids = new uint256[](3);
        for (uint256 i = 0; i < 3; i++) ids[i] = newIds[i];
    }
    ...
    function test_e2e_migrateToNewEntitiesAndDistribute() public {
        ...
        string[3] memory names = ["e2e-alpha", "e2e-bravo", "e2e-charlie"];
        uint256 reg = entity.minRegistrationStake();
        for (uint256 i = 0; i < 3; i++) {
            vm.prank(admin);
            entity.createEntity{value: reg}(IVanaPoolEntity.EntityRegistrationInfo({ownerAddress: owners[i], name: names[i]}));
        ...
        // ---- 2. block new stake into the old entity; exits and redelegate-out stay open ----
        ...
        // ---- 3. every live staker of entity 1 migrates into the new entities ----
        ...
        uint256 migrated; uint256 movedTotal;
        ...
        for (uint256 i = 0; i < 3; i++) if (staking.stakerEntities(sample, newIds[i]).shares > 0) sampleEntity = newIds[i];
        assertGt(sampleEntity, 0, "sample staker landed in a new entity");
        uint256 sampleBefore = _value(sample, sampleEntity);

        // ---- 4. fund the splitter (the admin/deployer wallet), 10% burn, two rounds ----
        ...
        for (uint256 i = 0; i < 3; i++) entity.processRewards(newIds[i]);
        uint256[3] memory lockedBefore; uint256[3] memory commBefore;
        for (uint256 i = 0; i < 3; i++) { lockedBefore[i] = entity.entityStakerLockedRewardPool(newIds[i]); commBefore[i] = entity.entityAccruedCommission(newIds[i]); }
        vm.prank(admin);
        splitter.distribute(BUDGET, _ids()); // round 2: pays by principal-seconds since round 1

        uint256 paidToStakers; uint256 paidCommission;
        for (uint256 i = 0; i < 3; i++) {
            ...
            paidToStakers += dLocked; paidCommission += dComm;
        ...
        // ---- 5. owners claim their commission (entity pays through the treasury: SPENDER_ROLE) ----
        for (uint256 i = 0; i < 3; i++) {
        ...
        // ---- 6. rewards reach the stakers: vest the splitter track (7 d) + the 40% APY drip ----
        ...
        // ---- 7. burn: the reserve leaves the splitter to address(0) ----

...
describe("QueryEngine", () => {
  ...
  const deploy = async () => {
    ...
    await DLPRegistryMock.connect(dlp1Owner).updateDlpTreasuryAddress(
      1,
      dlp1Owner.address,
    );
    ...
    const queryEngineDeploy = await upgrades.deployProxy(
      ...
      [
        ...
        dataAccessTreasuryFactory.target,
      ],
      {
        kind: "uups",
      },
    ...
    await queryEngine
      .connect(owner)
      .grantRole(QUERY_ENGINE_ROLE, queryEngineTEE.address);

    await queryEngine
      .connect(owner)
      .updateDlpPaymentPercentage(dlpPaymentPercentage);
  ...
  describe("Setup", () => {
    ...
    it("should have correct params after deploy", async function () {
      ...
      (await queryEngine.hasRole(QUERY_ENGINE_ROLE, queryEngineTEE)).should.eq(
        true,
      );
      (await queryEngine.version()).should.eq(1);
      (await queryEngine.refinerRegistry()).should.eq(dataRefinerRegistry);
      (await queryEngine.dlpPaymentPercentage()).should.eq(
    ...
    it("should have correct treasury addresses after deploy", async function () {
      ...
      const queryEngineTreasuryInitializeData =
        dataAccessTreasuryIface.encodeFunctionData("initialize", [
          owner.address,
          queryEngine.target,
        ]);
      const queryEngineTreasuryProxyArgs =
        ethers.AbiCoder.defaultAbiCoder().encode(
          ["address", "bytes"],
          [dataAccessTreasuryFactory.target, queryEngineTreasuryInitializeData],
        );
      const queryEngineTreasuryProxyInitCode = ethers.solidityPacked(
        ["bytes", "bytes"],
        [beaconProxyFactory.bytecode, queryEngineTreasuryProxyArgs],
      );
      const queryEngineTreasuryProxyAddress = ethers.getCreate2Address(
        ...
        ethers.keccak256(
          ethers.solidityPacked(["address"], [queryEngine.target]),
        ),
        ethers.keccak256(queryEngineTreasuryProxyInitCode),
      );
      queryEngineTreasury.should.eq(queryEngineTreasuryProxyAddress);
      queryEngineTreasuryProxyAddress.should.eq(
        await dataAccessTreasuryFactory.getProxyAddress(
          queryEngineTreasuryInitializeData,
          queryEngine.target,
        ),
      );
    ...
    it("should grant or revoke roles when admin", async function () {
      await queryEngine.connect(owner).grantRole(MAINTAINER_ROLE, user1.address)
    ...
    it("should not initialize in implementation contract", async function () {
      const impl = await ethers.deployContract("QueryEngineImplementation");

      await impl
        .initialize(
          owner.address,
          dataRefinerRegistry.target,
          dataAccessTreasuryImpl.target,
        )
        .should.be.rejectedWith("InvalidInitialization()");
    });

    it("should pause and unpause only when maintainer", async function () {
      await queryEngine.connect(maintainer).pause().should.be.fulfilled;
      (await queryEngine.paused()).should.eq(true);

      await queryEngine.connect(maintainer).unpause().should.be.fulfilled;
    ...
    it("should updateRefinerRegistry only when maintainer", async function () {
      ...
      await queryEngine
        .connect(maintainer)
        .updateRefinerRegistry(newImpl.target).should.be.fulfilled;
      ...
      await queryEngine.connect(owner).updateRefinerRegistry(newImpl1.target)
        .should.be.fulfilled;
    ...
    it("should updateComputeEngine only when maintainer", async function () {
      ...
      await queryEngine.connect(maintainer).updateComputeEngine(newImpl.target)
        .should.be.fulfilled;
      ...
      await queryEngine.connect(owner).updateComputeEngine(newImpl1.target)
        .should.be.fulfilled;
    ...
    it("should updateQueryEngineTreasury only when maintainer", async function () {
      ...
      await queryEngine
        .connect(maintainer)
        .updateQueryEngineTreasury(newImpl.target).should.be.fulfilled;
      ...
      await queryEngine
        .connect(owner)
        .updateQueryEngineTreasury(newImpl1.target).should.be.fulfilled;
    ...
    it("should updateVanaTreasury only when maintainer", async function () {
      ...
      await queryEngine.connect(maintainer).updateVanaTreasury(newVanaTreasury)
        .should.be.fulfilled;
  ...
  describe("Permissions", () => {
    ...
    it("should updatePermissionApproval only when DLP owner", async function () {
      ...
      await queryEngine.connect(dlp1Owner).updatePermissionApproval(1, false)
        .should.be.fulfilled;
  ...
  describe("Payments", () => {
    ...
    beforeEach(async () => {
      ...
      const teePoolProxyAddress = await getTeePoolAddress(
        TeePoolType.Dedicated,
        HardwareType.GPU,
        maxUint80,
        await teePoolFactory.getAddress(),
      );
    ...
    it("should requestPaymentInVana when queryEngineTEE", async function () {
      (await computeEngine.balanceOf(user1, VanaToken)).should.eq(
        depositAmount,
      );
      (
      ...
      (await ethers.provider.getBalance(queryEngineTreasury.target)).should.eq(
        0,
      );
      ...
      const vanaTreasuryBalanceBefore =
        await ethers.provider.getBalance(vanaTreasury);

      await queryEngine
        .connect(queryEngineTEE)
        .requestPaymentInVana(dataAccessCost, jobId1, refinerId1)
        .should.emit(queryEngine, "PaymentReceived")
        .withArgs(VanaToken, dataAccessCost, jobId1, refinerId1);

      (await computeEngine.balanceOf(user1, VanaToken)).should.eq(
        depositAmount - dataAccessCost,
      );
      (
        await ethers.provider.getBalance(computeEngineTreasury.target)
      ).should.eq(depositAmount - dataAccessCost);

      const dlp1Payment =
        (dataAccessCost * dlpPaymentPercentage) / ONE_HUNDRED_PERCENT;
      const vanaPayment = dataAccessCost - dlp1Payment;

      (await ethers.provider.getBalance(vanaTreasury)).should.eq(
        vanaTreasuryBalanceBefore + vanaPayment,
      );
      (await ethers.provider.getBalance(queryEngineTreasury.target)).should.eq(
        dlp1Payment,
      );
      ...
      tx.should
        .emit(queryEngine, "DlpPaymentClaimed")
      ...
      (await ethers.provider.getBalance(dlp1Owner)).should.eq(
        dlp1OwnerBalanceBefore + dlp1Payment - txReceipt.fee,
      );
    });
    ...
    it("should revert when token is not VanaToken or ERC20", async function () {
      ...
      await queryEngine
        .connect(queryEngineTEE)
        .requestPayment(user1.address, dataAccessCost, metadata).should.be
        .reverted;
    });
    ...
    it("should revert when the payment is not received", async function () {
      ...
      const maliciousComputeEngine = await (
        await ethers.getContractFactory("ComputeEngineMaliciousMock")
      ).deploy();

      await queryEngine
        .connect(maintainer)
        .updateComputeEngine(maliciousComputeEngine.target).should.be.fulfilled;
    ...
    it("should revert when reentrancy", async function () {
      ...
      const maliciousComputeEngine = await (
        await ethers.getContractFactory("ComputeEngineMaliciousMock2")
      ).deploy();
    ...
    it("should revert when dlpTreasuryAddress is not set", async function () {
      ...
      await queryEngine
        ...
        .should.emit(queryEngine, "PaymentReceived")
        .withArgs(VanaToken, dataAccessCost, jobId4, refinerId3);
      ...
      await DLPRegistryMock.connect(dlp2Owner).updateDlpTreasuryAddress(
        dlpId2,
        dlp2Treasury,
      );

      const dlp2TreasuryBalanceBefore =
        await ethers.provider.getBalance(dlp2Treasury);
      ...
      (await ethers.provider.getBalance(dlp2Treasury)).should.eq(
        dlp2TreasuryBalanceBefore + dlp2Payment,
      );
    });
    ...
    it("should not claimDlpPayment when queryEngineTreasury paused", async function () {
      ...
      await queryEngineTreasury.connect(owner).pause();
    ...
    it("should not requestPaymentInVana when computeEngineTreasury paused", async function () {
      await computeEngineTreasury.connect(owner).pause();
  ...
  describe("Treasury", () => {
    ...
    it("should have correct roles", async function () {
      (await queryEngineTreasury.hasRole(DEFAULT_ADMIN_ROLE, owner)).should.eq(
        true,
      );
      (await queryEngineTreasury.hasRole(CUSTODIAN_ROLE, owner)).should.eq(
        true,
      );
      (
        await queryEngineTreasury.hasRole(CUSTODIAN_ROLE, queryEngine.target)
      ).should.eq(true);
      (
        await queryEngineTreasury.hasRole(
          DEFAULT_ADMIN_ROLE,
          queryEngine.target,
        )
      ).should.eq(false);
    ...
    it("should updateCustodian only when admin", async function () {
      ...
      (
        await queryEngineTreasury.hasRole(DEFAULT_ADMIN_ROLE, newCustodian)
      ).should.eq(false);
      (
        await queryEngineTreasury.hasRole(CUSTODIAN_ROLE, newCustodian)
      ).should.eq(false);

      await queryEngineTreasury.connect(owner).updateCustodian(newCustodian)
        .should.be.fulfilled;
    ...
    it("should grant or revoke CUSTODIAN_ROLE only when admin", async function () {
      ...
      await queryEngineTreasury
        .connect(owner)
        .grantRole(CUSTODIAN_ROLE, newCustodian).should.be.fulfilled;
      (
        await queryEngineTreasury.hasRole(CUSTODIAN_ROLE, newCustodian)
      ).should.eq(true);
      ...
      await queryEngineTreasury
        .connect(owner)
        .revokeRole(CUSTODIAN_ROLE, newCustodian).should.be.fulfilled;
      (
        await queryEngineTreasury.hasRole(CUSTODIAN_ROLE, newCustodian)
      ).should.eq(false);
      (
    ...
    it("should transfer only when CUSTODIAN_ROLE", async function () {
      ...
      await queryEngineTreasury
        .connect(owner)
        .transfer(recoveryAddress, VanaToken, parseEther(2)).should.not.be
        .fulfilled;

      await queryEngineTreasury
        .connect(owner)
        .transfer(recoveryAddress, VanaToken, parseEther(1)).should.be
        .fulfilled;
      (await ethers.provider.getBalance(queryEngineTreasury.target)).should.eq(
        0,
      );
      (await ethers.provider.getBalance(recoveryAddress)).should.eq(
        parseEther(1),
      );

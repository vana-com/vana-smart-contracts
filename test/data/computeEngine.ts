...
describe("ComputeEngine", () => {
  ...
  const TeeAssignmentFailedSelector = ethers.id(
    "TeeAssignmentFailed(uint256,bytes)",
  );
  const TeeAssignmentSucceededSelector = ethers.id(
    "TeeAssignmentSucceeded(uint256,address,address)",
  );

  const getTeePoolAddress = async function (
    teePoolType: number,
    hardwareType: number,
    ...
    const salt = ethers.keccak256(
      ethers.solidityPacked(["address"], [deployer]),
    );
    ...
    const proxyArgs = ethers.AbiCoder.defaultAbiCoder().encode(
      ["address", "bytes"],
      [teePoolProxyFactory.target, initializeData],
    );
    const proxyInitCode = ethers.solidityPacked(
      ["bytes", "bytes"],
      [beaconProxyFactory.bytecode, proxyArgs],
    );
  ...
  const deploy = async () => {
    ...
    DLPRegistryMock = await ethers.deployContract("DLPRegistryMock");
    ...
    const queryEngineDeploy = await upgrades.deployProxy(
      ...
      [
        owner.address,
        dataRefinerRegistry.target,
        dataAccessTreasuryProxyFactory.target,
      ],
      {
        kind: "uups",
      },
  ...
  describe("Setup", () => {
    ...
    it("should have correct params after deploy", async function () {
      ...
      (await computeEngine.hasRole(MAINTAINER_ROLE, maintainer)).should.eq(
        true,
      );
      (await computeEngine.version()).should.eq(1);
      (await computeEngine.instructionRegistry()).should.eq(
        computeInstructionRegistry,
      );
      (await computeEngine.queryEngine()).should.eq(queryEngine);
      (await computeEngine.computeEngineTreasury()).should.eq(
    ...
    it("should have correct treasury address after deploy", async function () {
      ...
      const computeEngineTreasuryInitializeData =
        dataAccessTreasuryIface.encodeFunctionData("initialize", [
          owner.address,
          computeEngine.target,
        ]);
      const computeEngineTreasuryProxyArgs =
      ...
      const computeEngineTreasuryProxyAddress = ethers.getCreate2Address(
        ...
        ethers.keccak256(
          ethers.solidityPacked(["address"], [computeEngine.target]),
        ),
    ...
    it("should not initialize in implementation contract", async function () {
      const impl = await ethers.deployContract("ComputeEngineImplementation");
    ...
    it("should pause and unpause only when maintainer", async function () {
      await computeEngine.connect(maintainer).pause().should.be.fulfilled;
      (await computeEngine.paused()).should.eq(true);

      await computeEngine.connect(maintainer).unpause().should.be.fulfilled;
    ...
    it("should updateQueryEngine only when maintainer", async function () {
      const newImpl = await ethers.deployContract("QueryEngineImplementation");

      await computeEngine.connect(maintainer).updateQueryEngine(newImpl.target)
        .should.be.fulfilled;
      (await computeEngine.queryEngine()).should.eq(newImpl.target);

      const newImpl1 = await ethers.deployContract("QueryEngineImplementation");
      newImpl1.should.not.eq(newImpl);

      await computeEngine.connect(owner).updateQueryEngine(newImpl1.target)
        .should.be.fulfilled;
    ...
    it("should updateTeePoolFactory only when maintainer", async function () {
      ...
      await computeEngine
        .connect(maintainer)
        .updateTeePoolFactory(newImpl.target).should.be.fulfilled;
      ...
      await computeEngine.connect(owner).updateTeePoolFactory(newImpl1.target)
        .should.be.fulfilled;
    ...
    it("should updateInstructionRegistry only when maintainer", async function () {
      ...
      await computeEngine
        .connect(maintainer)
        .updateInstructionRegistry(newImpl.target).should.be.fulfilled;
      ...
      await computeEngine
        .connect(owner)
        .updateInstructionRegistry(newImpl1.target).should.be.fulfilled;
  ...
  describe("TeePool Factory", () => {
    ...
    it("should createTeePool only when maintainer", async function () {
      // Ephemeral + Standard
      const ephemeralStandardAddress = await getTeePoolAddress(
        TeePoolType.Ephemeral,
        HardwareType.Standard,
        ephemeralTimeout,
        teePoolFactoryAddress,
      );
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Ephemeral, HardwareType.Standard)
      ...
      (
        await teePoolFactory.teePools(
          TeePoolType.Ephemeral,
          HardwareType.Standard,
        )
      ).should.eq(ephemeralStandardAddress);
      ...
      (
        await ephemeralStandardTeePool.hasRole(DEFAULT_ADMIN_ROLE, maintainer)
      ).should.eq(true);
      (await ephemeralStandardTeePool.teePoolType()).should.eq(
        TeePoolType.Ephemeral,
      );
      (await ephemeralStandardTeePool.hardwareType()).should.eq(
        HardwareType.Standard,
      );
      (await ephemeralStandardTeePool.maxTimeout()).should.eq(ephemeralTimeout);
      (await ephemeralStandardTeePool.computeEngine()).should.eq(
        computeEngine.target,
      );
      (
        await teePoolFactory.teePools(
      ...
      // Ephemeral + GPU
      const ephemeralGPUAddress = await getTeePoolAddress(
        TeePoolType.Ephemeral,
        HardwareType.GPU,
        ephemeralTimeout,
        teePoolFactoryAddress,
      ...
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Ephemeral, HardwareType.GPU)
        .should.emit(teePoolFactory, "TeePoolCreated")
        .withArgs(
          ephemeralGPUAddress,
          TeePoolType.Ephemeral,
      ...
      (
        await ephemeralGPUTeePool.hasRole(DEFAULT_ADMIN_ROLE, maintainer)
      ).should.eq(true);
      (await ephemeralGPUTeePool.teePoolType()).should.eq(
        TeePoolType.Ephemeral,
      );
      ...
      (await ephemeralGPUTeePool.computeEngine()).should.eq(
        computeEngine.target,
      );
      (
        await teePoolFactory.teePools(TeePoolType.Ephemeral, HardwareType.GPU)
      ).should.eq(ephemeralGPUAddress);

      // Persistent + Standard
      const persistentStandardAddress = await getTeePoolAddress(
      ...
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Persistent, HardwareType.Standard)
        .should.emit(teePoolFactory, "TeePoolCreated")
        .withArgs(
      ...
      (
        await persistentStandardTeePool.hasRole(DEFAULT_ADMIN_ROLE, maintainer)
      ).should.eq(true);
      (await persistentStandardTeePool.teePoolType()).should.eq(
        TeePoolType.Persistent,
      );
      (await persistentStandardTeePool.hardwareType()).should.eq(
        HardwareType.Standard,
      );
      (await persistentStandardTeePool.maxTimeout()).should.eq(
        persistentTimeout,
      );
      (await persistentStandardTeePool.computeEngine()).should.eq(
        computeEngine.target,
      );
      (
        await teePoolFactory.teePools(
      ...
      ).should.eq(persistentStandardAddress);

      // Persistent + GPU
      const persistentGPUAddress = await getTeePoolAddress(
        TeePoolType.Persistent,
        HardwareType.GPU,
        persistentTimeout,
        teePoolFactoryAddress,
      ...
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Persistent, HardwareType.GPU)
        .should.emit(teePoolFactory, "TeePoolCreated")
        .withArgs(
      ...
      (
        await persistentGPUTeePool.hasRole(DEFAULT_ADMIN_ROLE, maintainer)
      ).should.eq(true);
      (await persistentGPUTeePool.teePoolType()).should.eq(
        TeePoolType.Persistent,
      );
      ...
      (await persistentGPUTeePool.computeEngine()).should.eq(
        computeEngine.target,
      );
      (
        await teePoolFactory.teePools(TeePoolType.Persistent, HardwareType.GPU)
      ).should.eq(persistentGPUAddress);

      // Dedicated + Standard
      const dedicatedStandardAddress = await getTeePoolAddress(
      ...
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Dedicated, HardwareType.Standard)
        .should.emit(teePoolFactory, "TeePoolCreated")
        .withArgs(
      ...
      (
        await dedicatedStandardTeePool.hasRole(DEFAULT_ADMIN_ROLE, maintainer)
      ).should.eq(true);
      (await dedicatedStandardTeePool.teePoolType()).should.eq(
        TeePoolType.Dedicated,
      );
      (await dedicatedStandardTeePool.hardwareType()).should.eq(
        HardwareType.Standard,
      );
      (await dedicatedStandardTeePool.maxTimeout()).should.eq(maxUint80);
      (await dedicatedStandardTeePool.computeEngine()).should.eq(
        computeEngine.target,
      );
      (
        await teePoolFactory.teePools(
      ...
      ).should.eq(dedicatedStandardAddress);

      // Dedicated + GPU
      const dedicatedGPUAddress = await getTeePoolAddress(
        TeePoolType.Dedicated,
        HardwareType.GPU,
        maxUint80,
        teePoolFactoryAddress,
      ...
      (
        await dedicatedGPUTeePool.hasRole(DEFAULT_ADMIN_ROLE, maintainer)
      ).should.eq(true);
      (await dedicatedGPUTeePool.teePoolType()).should.eq(
        TeePoolType.Dedicated,
      );
      ...
      (await dedicatedGPUTeePool.computeEngine()).should.eq(
        computeEngine.target,
      );
      (
        await teePoolFactory.teePools(TeePoolType.Dedicated, HardwareType.GPU)
      ).should.eq(dedicatedGPUAddress);
    ...
    it("should not createTeePool when invalid TeePoolType", async function () {
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.None, HardwareType.Standard)
        .should.be.rejectedWith("InvalidTeePoolParams()");

      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Ephemeral, HardwareType.None)
        .should.be.rejectedWith("InvalidTeePoolParams()");

      await teePoolFactory
        .connect(maintainer)
        .createTeePool(4, HardwareType.Standard).should.be.reverted;

      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Ephemeral, 3).should.be.reverted;
    });

    it("should not create duplicate TeePool", async function () {
      await teePoolFactory
        .connect(maintainer)
        .createTeePool(TeePoolType.Ephemeral, HardwareType.Standard).should.be
        .fulfilled;
    ...
    it("should updateEphemeralTimeout only when maintainer", async function () {
      ...
      await teePoolFactory
        .connect(maintainer)
        .updateEphemeralTimeout(newEphemeralTimeout).should.be.fulfilled;
      ...
      (await ephemeralStandardTeePool.maxTimeout()).should.eq(
        newEphemeralTimeout,
      );
    ...
    it("should updatePersistentTimeout only when maintainer", async function () {
      ...
      await teePoolFactory
        .connect(maintainer)
        .updatePersistentTimeout(newPersistentTimeout).should.be.fulfilled;
      (await teePoolFactory.persistentTimeout()).should.eq(
        newPersistentTimeout,
      );
  ...
  describe("TeePool", () => {
    ...
    const teeES1Params = ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "string"],
      ["urlES1", "publicKeyES1"],
    );
    const teeES2Params = ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "string"],
      ["urlES2", "publicKeyES2"],
    );
    const teeES3Params = ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "string"],
      ["urlES3", "publicKeyES3"],
    );
    const teeDS1Params = ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "string"],
      ["urlDS1", "publicKeyDS1"],
    );
    const teeDS2Params = ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "string"],
      ["urlDS2", "publicKeyDS2"],
    );
    ...
    beforeEach(async () => {
      ...
      await computeInstructionRegistry
        .connect(user1)
        .addComputeInstruction(instructionHash1, "instructionUrl1")
    ...
    it("should removeTee only when maintainer", async function () {
      ...
      await ephemeralStandardTeePool
        ...
        .removeTee(ephemeralStandardTee1.address)
        .should.be.rejectedWith(
          `TeeNotActive("${ephemeralStandardTee1.address}")`,
        );
      ...
      await dedicatedStandardTeePool
        ...
        .should.be.rejectedWith(
          `TeeNotActive("${dedicatedStandardTee1.address}")`,
        );
    });
    ...
    it("should submitJob to active Tees only", async function () {
      ...
      await computeEngine
        .connect(user1)
        .submitJob(ephemeralTimeout, false, instructionId1, {
          value: parseEther(10),
        })
        ...
        .withArgs(
          jobId1,
          ephemeralStandardTeePool.target,
          ephemeralStandardTee1.address,
        )
      ...
      await computeEngine
        ...
        .withArgs(
          jobId5,
          dedicatedStandardTeePool.target,
          dedicatedStandardTee2.address,
        )
  ...
  describe("Job Registry", () => {
    ...
    const getJobsCountTee = async (
      teePool: ComputeEngineTeePoolImplementation,
      teeAddress: any,
    ) => {
    ...
    it("should registerJob without TeePool", async function () {
      ...
      (await computeEngine.jobs(jobId1)).should.deep.eq([
        ...
        await ethers.provider
          .getBlock("latest")
          .then((block) => (block ? block.timestamp : 0)), // addedTimestamp
        "", // statusMessage
      ...
      let tx = await computeEngine.connect(user2).submitJob(
        maxTimeout, // maxTimeout = 5s
        gpuRequired, // gpuRequired = true
        instructionId1, // computeInstructionId = 1
        { value: depositAmount },
      ...
      tx.should
        .emit(computeEngine, "JobRegistered")
      ...
      (await ethers.provider.getBalance(user2)).should.eq(
        user2BalanceBefore - depositAmount - txReceipt.fee,
      );
      (await computeEngine.balanceOf(user2, VanaToken)).should.eq(
        depositAmount,
      );
      (
        await ethers.provider.getBalance(computeEngineTreasury.target)
      ).should.eq(depositAmount);
    });
    ...
    it("should registerJob with empty TeePool", async function () {
      ...
      await computeEngine
        ...
        .withArgs(
          jobId1,
          getErrorSelectorWithArgs(
            "NoActiveTee(address)",
            ephemeralGPUTeePool.target,
          ),
        );
    ...
    it("should submitJob with non-empty TeePool", async function () {
      ...
      [ephemeralGPUTee1, ephemeralGPUTee2, ephemeralGPUTee3] =
        await ethers.getSigners();
      ...
      const tee1Params = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["url1", "publicKey1"],
      );
      await ephemeralGPUTeePool
      ...
      const tee2Params = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["url2", "publicKey2"],
      );
      await ephemeralGPUTeePool
      ...
      (
        await getJobsCountTee(ephemeralGPUTeePool, ephemeralGPUTee1.address)
      ).should.eq(0);
      (
        await getJobsCountTee(ephemeralGPUTeePool, ephemeralGPUTee2.address)
      ).should.eq(0);
      (
        await getJobsCountTee(ephemeralGPUTeePool, ephemeralGPUTee3.address)
      ).should.eq(0);
      ...
      const tee3Params = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["url3", "publicKey3"],
      );
      await ephemeralGPUTeePool
    ...
    it("should resubmitJob when Tee is available", async function () {
      ...
      const addedTimestamp = await ethers.provider
        .getBlock("latest")
        .then((block) => (block ? block.timestamp : 0));
      (await computeEngine.jobsCount()).should.eq(1);
      (await computeEngine.jobs(jobId1)).should.deep.eq([
        user1.address, // ownerAddress
        maxTimeout, // maxTimeout
    ...
    it("should submitJobWithTee when Tee is available", async function () {
      ...
      (
        await getJobsCountTee(
          dedicatedStandardTeePool,
          dedicatedStandardTee1.address,
        )
      ).should.eq(1);
      (await computeEngine.jobs(jobId1)).should.deep.eq([
    ...
    it("should resubmitJobWithTee when the dedicated Tee is available", async function () {
      ...
      await computeEngine
        .connect(user1)
        .submitJob(maxTimeout, gpuRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId1, user1)
        .and.emit(computeEngine, "TeeAssignmentFailed")
        .withArgs(
          jobId1,
          getErrorSelectorWithArgs("TeeNotActive(address)", ethers.ZeroAddress),
        );
    ...
    it("should updateJobStatus only when assigned Tee", async function () {
      ...
      (await computeEngine.jobs(jobId1)).teeAddress.should.eq(
        ethers.ZeroAddress,
      );
      ...
      (await computeEngine.jobs(jobId2)).teeAddress.should.eq(
        ephemeralGPUTee1.address,
      );
      (await computeEngine.jobs(jobId2)).teePoolAddress.should.eq(
        ephemeralGPUTeePool.target,
      );
      ...
      await computeEngine
        ...
        .should.be.rejectedWith(
          `InvalidStatusTransition(${JobStatus.Submitted}, ${JobStatus.Registered})`,
        );
      ...
      await computeEngine
        ...
        .should.be.rejectedWith(
          `InvalidStatusTransition(${JobStatus.Running}, ${JobStatus.Submitted})`,
        );
      ...
      await computeEngine
        ...
        .should.be.rejectedWith(
          `InvalidStatusTransition(${JobStatus.Running}, ${JobStatus.Canceled})`,
        );
      ...
      await computeEngine
        ...
        .should.be.rejectedWith(
          `InvalidStatusTransition(${JobStatus.Canceled}, ${JobStatus.Failed})`,
        );
      ...
      (await computeEngine.jobs(jobId3)).teeAddress.should.eq(
        ephemeralGPUTee1.address,
      );

      (await ephemeralGPUTeePool.isTee(ephemeralGPUTee1.address)).should.eq(
        true,
      );
      await ephemeralGPUTeePool
        .connect(maintainer)
    ...
    it("should submitJob to correct TeePool and Tee", async function () {
      ...
      const teeESParams = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["urlES", "publicKeyES"],
      );
      const teeEGParams = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["urlEG", "publicKeyEG"],
      );
      const teePSParams = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["urlPS", "publicKeyPS"],
      );
      const teePGParams = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["urlPG", "publicKeyPG"],
      );
      const teeDSParams = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["urlDS", "publicKeyDS"],
      );
      const teeDGParams = ethers.AbiCoder.defaultAbiCoder().encode(
        ["string", "string"],
        ["urlDG", "publicKeyDG"],
      );
      ...
      await computeEngine
        .connect(user1)
        .submitJob(ephemeralTimeout, gpuNotRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId1, user1);
      (await computeEngine.jobs(jobId1)).status.should.eq(JobStatus.Registered);
      (await computeEngine.jobs(jobId1)).teeAddress.should.eq(
      ...
      await computeEngine
        .connect(user1)
        .submitJob(ephemeralTimeout, gpuRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId2, user1);
      ...
      await computeEngine
        .connect(user1)
        .submitJob(persistentTimeout, gpuNotRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId3, user1);
      ...
      await computeEngine
        .connect(user1)
        .submitJob(persistentTimeout, gpuRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId4, user1);
      (await computeEngine.jobs(jobId4)).status.should.eq(JobStatus.Registered);
      (await computeEngine.jobs(jobId4)).teeAddress.should.eq(
        ethers.ZeroAddress,
      );

      await computeEngine
        .connect(user1)
        .submitJob(2 * persistentTimeout, gpuNotRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId5, user1);
      (await computeEngine.jobs(jobId5)).status.should.eq(JobStatus.Registered);
      (await computeEngine.jobs(jobId5)).teeAddress.should.eq(
        ethers.ZeroAddress,
      );

      await computeEngine
        .connect(user1)
        .submitJob(2 * persistentTimeout, gpuRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId6, user1);
      (await computeEngine.jobs(jobId6)).status.should.eq(JobStatus.Registered);
      (await computeEngine.jobs(jobId6)).teeAddress.should.eq(
        ethers.ZeroAddress,
      );
      ...
      (
        await ephemeralStandardTeePool.isTee(ephemeralStandardTee.address)
      ).should.eq(true);
      (
        await getJobsCountTee(
          ephemeralStandardTeePool,
          ephemeralStandardTee.address,
        )
      ).should.eq(0);
      ...
      (await ephemeralGPUTeePool.isTee(ephemeralGPUTee.address)).should.eq(
        true,
      );
      (
        await getJobsCountTee(ephemeralGPUTeePool, ephemeralGPUTee.address)
      ).should.eq(0);
      ...
      (
        await persistentStandardTeePool.isTee(persistentStandardTee.address)
      ).should.eq(true);
      (
        await getJobsCountTee(
          persistentStandardTeePool,
          persistentStandardTee.address,
        )
      ).should.eq(0);
      ...
      (await persistentGPUTeePool.isTee(persistentGPUTee.address)).should.eq(
        true,
      );
      (
        await getJobsCountTee(persistentGPUTeePool, persistentGPUTee.address)
      ).should.eq(0);
      ...
      (await computeEngine.jobs(jobId7)).teeAddress.should.eq(
        ephemeralStandardTee.address,
      );
      ...
      (await computeEngine.jobs(jobId8)).teeAddress.should.eq(
        ephemeralGPUTee.address,
      );
      ...
      (await computeEngine.jobs(jobId9)).teeAddress.should.eq(
        persistentStandardTee.address,
      );
      ...
      (await computeEngine.jobs(jobId10)).teeAddress.should.eq(
        persistentGPUTee.address,
      );
      ...
      await computeEngine
        .connect(user1)
        .submitJob(maxUint80, gpuNotRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId11, user1);
      (await computeEngine.jobs(jobId11)).status.should.eq(
        JobStatus.Registered,
      );
      (await computeEngine.jobs(jobId11)).teeAddress.should.eq(
        ethers.ZeroAddress,
      );

      await computeEngine
        .connect(user1)
        .submitJob(maxUint80, gpuRequired, instructionId1)
        .should.emit(computeEngine, "JobRegistered")
        .withArgs(jobId12, user1);
      (await computeEngine.jobs(jobId12)).status.should.eq(
        JobStatus.Registered,
      );
      (await computeEngine.jobs(jobId12)).teeAddress.should.eq(
        ethers.ZeroAddress,
      );
      ...
      (await computeEngine.jobs(jobId13)).teeAddress.should.eq(
        dedicatedStandardTee.address,
      );
      ...
      (await computeEngine.jobs(jobId14)).teeAddress.should.eq(
        dedicatedGPUTee.address,
      );
      ...
      (
        await getJobsCountTee(dedicatedGPUTeePool, dedicatedGPUTee.address)
      ).should.eq(4);
    });
    ...
    it("should cancelJob when owner", async function () {
      ...
      let addedTimestamp = await ethers.provider
        .getBlock("latest")
        .then((block) => (block ? block.timestamp : 0));
      ...
      addedTimestamp = await ethers.provider
        .getBlock("latest")
        .then((block) => (block ? block.timestamp : 0));
      ...
      await computeEngine
        ...
        .should.emit(computeEngine, "JobCanceled")
        .withArgs(jobId3)
        .and.emit(dedicatedStandardTeePool, "JobRemoved")
        .withArgs(jobId3);
  ...
  describe("Payment", function () {
    ...
    beforeEach(async () => {
      ...
      (await erc20Mock.balanceOf(dlp1Owner.address)).should.eq(
    ...
    it("should deposit and withdraw VANA", async function () {
      ...
      let computeEngineTreasuryVanaBalance = await ethers.provider.getBalance(
        computeEngineTreasury.target,
      );
      ...
      (
        await ethers.provider.getBalance(computeEngineTreasury.target)
      ).should.eq(user1DepositAmount);
      (await computeEngine.balanceOf(user1.address, VanaToken)).should.eq(
        user1DepositAmount,
      );
      ...
      (
        await computeEngine.balanceOf(user1.address, erc20Mock.target)
      ).should.eq(0);
      ...
      (
        await ethers.provider.getBalance(computeEngineTreasury.target)
      ).should.eq(computeEngineTreasuryVanaBalance);
      (await computeEngine.balanceOf(user1.address, VanaToken)).should.eq(
        user1DepositAmount - user1WithdrawAmount,
      );
    ...
    it("should deposit and withdraw ERC20", async function () {
      ...
      let user1DepositAmount: bigint = parseEther(100);
      let computeEngineTreasuryERC20Balance = await erc20Mock.balanceOf(
        computeEngineTreasury.target,
      );
      ...
      await computeEngine
        ...
        .should.be.rejectedWith(
          `ERC20InsufficientAllowance("${computeEngine.target}", 0, ${user1DepositAmount})`,
        );
      ...
      computeEngineTreasuryERC20Balance =
        computeEngineTreasuryERC20Balance + user1DepositAmount;

      (await erc20Mock.balanceOf(user1.address)).should.eq(user1ERC20Balance);
      (await erc20Mock.balanceOf(computeEngine.target)).should.eq(0);
      (await erc20Mock.balanceOf(computeEngineTreasury.target)).should.eq(
        user1DepositAmount,
      );
      (
        await computeEngine.balanceOf(user1.address, erc20Mock.target)
      ...
      computeEngineTreasuryERC20Balance =
        computeEngineTreasuryERC20Balance - user1WithdrawAmount;
      ...
      (
        await computeEngine.balanceOf(user1.address, erc20Mock.target)
      ).should.eq(user1DepositAmount - user1WithdrawAmount);
    ...
    it("should executePaymentRequest from queryEngine", async function () {
      ...
      [mockQueryEngine, ephemeralStandardTee1, dedicatedStandardTee2] =
        await ethers.getSigners();

      const requestedAmount = parseEther(100);
      const requestParams1 = ethers.AbiCoder.defaultAbiCoder().encode(
        ["uint256", "uint256"],
        [jobId1, dlpId1],
      );
      const requestParams2 = ethers.AbiCoder.defaultAbiCoder().encode(
        ["uint256", "uint256"],
        [jobId2, dlpId1],
      );
      const requestParams3 = ethers.AbiCoder.defaultAbiCoder().encode(
        ["uint256", "uint256"],
        [jobId3, dlpId2],
      );
      ...
      await computeEngine
        .connect(maintainer)
        .updateQueryEngine(mockQueryEngine.address).should.be.fulfilled;
      (await computeEngine.queryEngine()).should.eq(mockQueryEngine.address);

      // Invalid jobId format
      await computeEngine
        .connect(mockQueryEngine)
        .executePaymentRequest(VanaToken, requestedAmount, "0x").should.not.be
        .fulfilled;
      ...
      let mockQueryEngineBalance = await ethers.provider.getBalance(
        mockQueryEngine.address,
      );
      let tx = await computeEngine
        .connect(mockQueryEngine)
        .executePaymentRequest(VanaToken, requestedAmount, requestParams1);
      tx.should
        .emit(computeEngine, "PaymentExecuted")
        .withArgs(jobId1, VanaToken, requestedAmount);
      let txReceipt = await getReceipt(tx);
      mockQueryEngineBalance =
        mockQueryEngineBalance + requestedAmount - txReceipt.fee;
      computeEngineTreasuryVanaBalance =
        computeEngineTreasuryVanaBalance - requestedAmount;

      (await computeEngine.balanceOf(user1.address, VanaToken)).should.eq(
        depositAmount - requestedAmount,
      );
      (await ethers.provider.getBalance(mockQueryEngine.address)).should.eq(
        mockQueryEngineBalance,
      );
      (
        await ethers.provider.getBalance(computeEngineTreasury.target)
      ...
      (await computeEngine.balanceOf(user2.address, VanaToken)).should.eq(
        depositAmount,
      );
      ...
      (await erc20Mock.balanceOf(mockQueryEngine.address)).should.eq(
        requestedERC20Amount,
      );
      (await erc20Mock.balanceOf(computeEngineTreasury.target)).should.eq(
        depositAmount - requestedERC20Amount,
      );
      (
        await computeEngine.balanceOf(user1.address, erc20Mock.target)
      ).should.eq(depositAmount - requestedERC20Amount);
    });

    it("should nonReentrant", async function () {
      const attacker = await (
        await ethers.getContractFactory("ComputeEngineMaliciousContract", {
          signer: user1,
        })
      ).deploy(computeEngine.target);

      const user1DepositAmount = parseEther(100);
      await attacker.connect(user1).deposit({ value: user1DepositAmount });
      (await computeEngine.balanceOf(attacker.target, VanaToken)).should.eq(
        user1DepositAmount,
      );
      ...
      // No reentrancy
      await attacker.connect(user1).withdraw(user1DepositAmount);
      (await computeEngine.balanceOf(attacker.target, VanaToken)).should.eq(0);
      (await ethers.provider.getBalance(attacker.target)).should.eq(
        user1DepositAmount,
      );
    });
    ...
    it("should not deposit/withdraw/executePaymentRequest when paused", async function () {
      ...
      (await computeEngine.balanceOf(user1.address, erc20Mock)).should.eq(
        depositAmount,
      );

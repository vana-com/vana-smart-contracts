...
describe("DAT", function () {
    ...
    return () => {
      describe("Token Functionality", () => {
        ...
        it("should have max cap when init cap is 0", async function () {
          ...
          const datToken = (await ethers.getContractAt(
            "DAT",
            datTokenAddress,
          )) as DAT;
        ...
        it("should revert when initializing with incorrect parameters", async function () {
          ...
          await datToken
            .initialize(
              ...
              owner.address,
              0,
              [ethers.ZeroAddress, beneficiary2],
              [amount1, amount2],
            )
          ...
          await datToken
            .initialize(
              ...
              owner.address,
              0,
              [beneficiary1, ethers.ZeroAddress],
              [amount1, amount2],
            )
            .should.be.rejectedWith(
              `ERC20InvalidReceiver("${ethers.ZeroAddress}")`,
        ...
        it("should mint by minter role only", async function () {
          ...
          (await datToken.totalSupply()).should.eq(
        ...
        it("should not mint over cap", async function () {
          ...
          await datToken
            ...
            .mint(user1.address, mintAmount)
            .should.be.rejectedWith(
              `ERC20ExceededCap(${tokenCap + 1n}, ${tokenCap})`,
            );

          await datToken
            .connect(owner)
            .mint(user1.address, tokenCap - totalSupplyBefore);
          (await datToken.balanceOf(user1.address)).should.eq(
            balanceBefore + tokenCap - totalSupplyBefore,
          );
        ...
        it("should transfer tokens", async function () {
          ...
          await datToken
            .connect(user1)
            .transfer(ethers.ZeroAddress, transferAmount)
            .should.be.rejectedWith(
              `ERC20InvalidReceiver("${ethers.ZeroAddress}")`,
            );

          const excessiveTransferAmount =
            (await datToken.balanceOf(user1.address)) + 1n;
          await datToken
            .connect(user1)
            .transfer(user2.address, excessiveTransferAmount)
            .should.be.rejectedWith(
      ...
      describe("Block List Functionality", () => {
        ...
        it("should unblock addresses only when admin", async function () {
          ...
          const beneficiary1BalanceBefore = await datToken.balanceOf(
          ...
          await datToken
            .connect(beneficiary1)
            .transfer(user1.address, transferAmount2).should.not.be.rejected;
          (await datToken.balanceOf(user1.address)).should.eq(
        ...
        it("should reject transferFrom to/from blocked addresses", async function () {
          ...
          // Try to transfer from user1 to user2 - should fail
          await datToken.connect(user1).approve(user2.address, parseEther(10));
          ...
          await datToken
            .connect(user2)
            .transferFrom(user1.address, user3.address, parseEther(10)).should
            .not.be.rejected;

          // Try to transfer from user2 to user1 - should fail
          await datToken.connect(user2).approve(user1.address, parseEther(10));
      ...
      describe("Role Management", () => {
        ...
        it("should allow admin to renounce roles", async function () {
          ...
          await datToken
            .connect(user1)
            .renounceRole(MINTER_ROLE, user1.address);
    ...
    return () => {
      describe("Voting Functionality", () => {
        ...
        it("should track voting power when tokens are transferred", async function () {
          ...
          (await datVotesToken.getVotes(user2.address)).should.eq(
            transferAmount,
          );
        });
        ...
        it("should handle delegation to another address", async function () {
          ...
          (await datVotesToken.getVotes(user3.address)).should.eq(
            amount1 + amount2,
          );
        });
      ...
      describe("ERC20Permits Functionality", () => {
        it("should support ERC20 permit", async function () {
          ...
          (
            await datVotesToken.allowance(owner.address, user1.address)
          ).should.eq(mintAmount);
  ...
  function testDATPausable(datContractName: string) {
    ...
    beforeEach(async () => {
      await deploy(datContractName);
      datPausableToken = await ethers.getContractAt(
        "DATPausable",
        datToken.target,
      );
    });
      ...
      describe("Pausing Functionality", () => {
        ...
        it("should allow admin to pause the token", async function () {
          ...
          await datPausableToken
            .connect(owner)
            .pause()
            .should.emit(datPausableToken, "Paused");
        ...
        it("should allow admin to unpause the token", async function () {
          ...
          await datPausableToken
            .connect(user1)
            .transfer(user2.address, parseEther(10)).should.not.be.rejected;
        });
...
describe("DATFactory + VestingWallet", () => {
  ...
  async function deploy() {
    ...
    const factoryDeploy = await upgrades.deployProxy(
      ...
      [
        ...
        maxCap,
        datImplementation.target,
        datVotesImplementation.target,
        datPausableImplementation.target,
      ],
      {
        kind: "uups",
      },
  ...
  async function createTokenWithVesting() {
    ...
    const tx = await datFactory.connect(owner).createToken({
      datType: 0,
      name: tokenName,
      symbol: tokenSymbol,
      cap: tokenCap,
  ...
  describe("Vesting Functionality", () => {
    it("should correctly set up vesting wallets", async function () {
      ...
      // Check beneficiaries
      (await vestingWallet1.owner()).should.eq(vestingSchedules[0].beneficiary);
      (await vestingWallet2.owner()).should.eq(vestingSchedules[1].beneficiary);
      ...
      (
        await vestingWallet1["vestedAmount(address,uint64)"](
          datToken.target,
          start1,
        )
      ).should.eq(0);
      ...
      (
        await vestingWallet2["vestedAmount(address,uint64)"](
          datToken.target,
          start2,
        )
      ).should.eq(0);
      ...
      (await vestingWallet1["releasable(address)"](datToken.target)).should.eq(
        0,
      );
      (await vestingWallet2["releasable(address)"](datToken.target)).should.eq(
        0,
      );
    });
    ...
    it("should release tokens according to vesting schedule", async function () {
      ...
      let actualRelease = await vestingWallet1["releasable(address)"](
        datToken.target,
      );
      actualRelease.should.eq(expectedRelease);

      // Release tokens
      await vestingWallet1
        .connect(beneficiary1)
        ["release(address)"](datToken.target);

      block = await ethers.provider.getBlock("latest");
      expectedRelease =
        (amount1 * (BigInt(block!.timestamp) - start1 - cliff1)) /
        (duration1 - cliff1);

      // Beneficiary should have received tokens
      let releasedAmount = await vestingWallet1["released(address)"](
        datToken.target,
      );
      ...
      const halfExpectedRelease =
        (amount1 * halfTimeElapsed) / effectiveDuration1;

      // Released amount should be deducted
      actualRelease = await vestingWallet1["releasable(address)"](
        datToken.target,
      );
      ...
      releasedAmount = await vestingWallet1["released(address)"](
        datToken.target,
      );
      releasedAmount.should.eq(expectedRelease);
      (await datToken.balanceOf(beneficiary1)).should.eq(expectedRelease);
  ...
  describe("DATFactory", () => {
    describe("Setup", () => {
      it("should have correct parameters after deployment", async function () {
        ...
        (await datFactory.datTemplates(1)).should.eq(
          datVotesImplementation.target,
        );
        (await datFactory.datTemplates(2)).should.eq(
          datPausableImplementation.target,
        );
      });
    ...
    describe("Token Creation", () => {
      ...
      it("should reject token creation with postCliffDuration=0", async function () {
        ...
        await datFactory.connect(owner).createToken({
          datType: 0,
          name: tokenName,
          symbol: tokenSymbol,
          cap: tokenCap,

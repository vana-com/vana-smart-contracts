...
contract DLPRootCoreCoreImplementationOld is
    UUPSUpgradeable,
    PausableUpgradeable,
    ...
    DLPRootCoreStorageV1
{
    ...
    function _calculateStakeRewardUntilEpoch(
        ...
        while (epochToClaim > 0 && epochToClaim <= lastEpochToClaim) {
            ...
            uint256 rewardAmount = (((epochDlp.rewardAmount * epochDlp.stakersRewardAmount) / 100e18) * stakeScore) /
    ...
    function _finalizeEpoch(uint256 epochId) internal {
        ...
        for (index = 0; index < topDlps.length; index++) {
            ...
            if (dlp.registrationBlockNumber > epoch.startBlock) {
                epochDlp.stakersRewardAmount = dlp.stakersPercentageCheckpoints.at(0)._value;
            } else {
                epochDlp.stakersRewardAmount = dlp.stakersPercentageCheckpoints.upperLookup(
            ...
            if (epochDlp.stakersRewardAmount < 100e18) {
                (bool success, ) = dlp.treasuryAddress.call{
                    value: (epochDlp.rewardAmount * (100e18 - epochDlp.stakersRewardAmount)) / 100e18
                }("");

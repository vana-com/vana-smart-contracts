...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const implementationDeploy = await deployments.deploy(implementationContractName, {
    ...
    deterministicDeployment: ethers.keccak256(ethers.toUtf8Bytes("VanaPoolStakingImplementation-v4")), log: true, gasPrice: gasPrice.toString(), nonce: deployOverrides.nonce,
  });

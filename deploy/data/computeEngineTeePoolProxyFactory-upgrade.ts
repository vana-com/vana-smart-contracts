...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const teePoolImplementationFactory = await ethers.getContractFactory(
    implementationContractName,
  );
  await upgrades.upgradeBeacon(
    teePoolProxyBeacon.address,
    teePoolImplementationFactory,
  );

  await delay(6000);

  const newImplementationAddress =
    await upgrades.beacon.getImplementationAddress(teePoolProxyBeacon.address);

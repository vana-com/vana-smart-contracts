...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const dataAccessTreasuryProxyFactoryDeploy = await deployments.get(
    "DataAccessTreasuryProxyFactory",
  );

  const computeInstructionRegistryDeploy = await deployments.get(
    "ComputeInstructionRegistryProxy",
  );

  const computeEngineTeePoolFactoryDeploy = await deployments.get(
    "ComputeEngineTeePoolFactoryProxy",
  );
  ...
  console.log(
    ...
    computeInstructionRegistryDeploy.address,
  );
  console.log(
    "computeEngineTeePoolFactory:",
    computeEngineTeePoolFactoryDeploy.address,
  );
  console.log("Salt:", salt);

  const initializeParams = [
    ownerAddress,
    queryEngineDeploy.address,
    computeEngineTeePoolFactoryDeploy.address,
    dataAccessTreasuryProxyFactoryDeploy.address,
  ];
  ...
  const computeEngine = await ethers.getContractAt(
    "ComputeEngineImplementation",
    proxyDeploy.proxyAddress,
  );
  await computeEngine.updateInstructionRegistry(
    computeInstructionRegistryDeploy.address,
  );

  const queryEngine = await ethers.getContractAt(
    "QueryEngineImplementation",
    queryEngineDeploy.address,
  );
  await queryEngine.updateComputeEngine(proxyDeploy.proxyAddress);

  const computeEngineTeePoolFactory = await ethers.getContractAt(
    "ComputeEngineTeePoolFactoryImplementation",
    computeEngineTeePoolFactoryDeploy.address,
  );
  await computeEngineTeePoolFactory.updateComputeEngine(
    proxyDeploy.proxyAddress,
  );
  ...
  const computeEngineTreasuryInitializeData =
    dataTreasuryImplementationFactory.interface.encodeFunctionData(
      "initialize",
